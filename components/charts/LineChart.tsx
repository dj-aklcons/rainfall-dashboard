...
export default function LineChart({ series, height = 280, accent, mode = "hourly" }: Props) {
  ...
  const points = data.map((d, i) => [padL + i * step, padT + innerH - (d.value / max) * innerH] as [number, number]);
  const path = points.map(([x, y], i) => `${i === 0 ? "M" : "L"}${x},${y}`).join(" ");
  const last = points[points.length - 1];
  const area = `${path} L${last[0]},${padT + innerH} L${padL},${padT + innerH} Z`;

  const yTicks = [0, 0.25, 0.5, 0.75, 1].map((f) => ({ y: padT + innerH * (1 - f), label: f === 0 ? "0" : fmtMM(max * f) }));
  ...
  function onMove(clientX: number, rect: DOMRect) {
    ...
    const d = data[i];
    if (!d) return;
    setHover({ i, px: padL + i * step, py: padT + innerH - (d.value / max) * innerH, ...d });
  }
